...
そして自然とコードが綺麗な状態を保てます。Vue.jsではどこで何をさせるべきか、どこで何をさせないべきかという指針が、APの設計に埋め込まれています。しかし、それでも十分な柔軟性を持っているので、窮屈さを感じることはありません。
...
アプリケーションの段階的な要求の変化に対応できて、規模に合わせて必要なだけ拡張/縮小できる柔軟性を持つフレームワーク。
...
ではどうやってVue.jsであらゆる規模に対応するのか、それにはVue.jsが公式で提供しているライブラリやツールを使います。

-----

## Vue.jsをサポートする<br>ライブラリ/ツールを使う
...
Vue.js本体のアップデートにライブラリ/ツールも追従してサポートが続く。（これまでの対応を見る限り、本体の更新とほぼ同時に更新されている）
...
全部をいっぺんに覚える必要はない。使うものを必要なときに導入し、その都度覚えていけばいい。
...
Vue.jsがどういうものかは、すべて[公式のガイド](http://vuejs.org/v2/guide/)に詰め込まれています。
